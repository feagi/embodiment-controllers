#!/usr/bin/env python
"""
MuJoCo Ant Controller - Using FEAGI Python SDK
...
def main():
    ...
    parser.add_argument('--model_xml_path', default='./ant.xml', help='MuJoCo model path')
    args = parser.parse_args()

    print("🚀 MuJoCo Ant Controller (FEAGI Python SDK)")
    ...
    print("\n🔌 Connecting to FEAGI...")
    feagi_client = FeagiAgentClient("mujoco_ant_01", AgentType.BOTH)
    ...
    feagi_client.configure(
        ...
        motor_capability=(
            "servo",               # modality
            8,                     # output count (8 joints - 4 legs, 2 joints each)
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        while viewer.is_running() and time.time() - start_time < RUNTIME:
            ...
            if feagi_ok:
                ...
                try:
                    # Convert joint positions to neuron activations (skip first 7 DOF - free joint)
                    neuron_pairs = []
                    for i in range(8):
                        neuron_id = i
                        qpos_idx = i + 7  # Skip free joint (7 DOF)
                        if qpos_idx < len(data.qpos):
                            potential = float(data.qpos[qpos_idx] * 50.0)  # Scale to reasonable range
